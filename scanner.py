...
def format_market_cap(value):
    ...
    try:
        ...
        return "🔸 N/A "
    if value >= 1e12:
        return f"💎 <b>${value/1e12:.2f}T </b> "
    if value >= 1e9:
        return f"💎 <b>${value/1e9:.2f}B </b> "
    if value >= 1e6:
        return f"💎 <b>${value/1e6:.2f}M </b> "
    if value >= 1e3:
        return f"💎 <b>${value/1e3:.2f}K </b> "
...
def calculate_composite_score_and_levels(sig):
    ...
    if 58 <= rsi <= 72:
        ...
        rsi_score = 3
    # 5. Market Cap (10%)
    if 50_000_000 <= mc <= 800_000_000:
        mc_score = 10
    elif 800_000_000 < mc <= 3_000_000_000:
        mc_score = 7
...
def select_top_signals(stage2_signals, top_n=8, min_score=50):
    ...
    for sig in stage2_signals:
        try:
            ...
            if DEBUG_MODE:
                print(f"⚠️ Scoring error for {sig['symbol']}: {e}")
